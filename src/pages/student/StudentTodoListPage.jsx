  ...
  return (
    <>
      ...
      <Card>
        ...
        {todoStatus ? (
          <p className="mt-2 text-xs font-semibold text-ink/80">{todoStatus}</p>
        ) : null}

        {loadingTodoItems ? (
          <p className="mt-3 text-sm text-ink/75">Loading tasks...</p>
        ) : todoItems.length === 0 ? (
          ...
          <div className="mt-3 grid gap-2">
            {todoItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="flex items-start justify-between gap-3 rounded-xl border border-clay/30 bg-white/95 px-3 py-2"
                >
                  ...
                  <button
                    ...
                    disabled={disableItemActions}
                    className="rounded-full border border-clay/35 bg-white px-3 py-1 text-[11px] font-semibold text-ink/75 hover:border-clay/50 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {isRemoving
                      ? "Removing..."
                      : isToggling
